...
export class UsersPage {
  ...
  row(name: string) { return this.rows.filter({ has: this.page.getByText(name, { exact: true }) }); }
  status(name: string, status: UserStatus | 'Updating status') { return this.row(name).getByRole('button', { name: status, exact: true }); }
  metric(name: 'Total users' | 'Active' | 'Pending' | 'Admins') {
    return this.page.locator('.metric-row article').filter({ has: this.page.getByText(name, { exact: true }) }).locator('strong');
  }
  async filter(role: UserRole | 'all') { await this.roleFilter.selectOption(role); }
  async select(name: string, checked = true) { await this.row(name).getByRole('checkbox').setChecked(checked); }
  async add() { await this.page.getByTestId('add-user-button').click(); }
  async edit(name: string) { await this.row(name).getByRole('button', { name: 'Edit', exact: true }).click(); }
  async deleteMenu(name: string) { await this.row(name).getByRole('button', { name: `Delete ${name}`, exact: true }).click(); }
}
