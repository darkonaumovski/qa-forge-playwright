...
for (const [id, name, from, to, active, pending] of [
  ...
  test(`QF-STATUS-${id} ${from} becomes ${to} with async feedback and metrics @smoke`, async ({
    users,
    shell,
  }) => {
    ...
    await expect(users.metric('Active')).toHaveText(active);
    await expect(users.metric('Pending')).toHaveText(pending);
...
test('QF-STATUS-04 Status changes affect only the chosen user', async ({ users }) => {
  ...
  await expect(users.status('Ava Rodriguez', 'Active')).toBeVisible();
});

test('QF-STATUS-05 Status changes can be reversed', async ({ users }) => {
  await users.status('Sofia Anders', 'Suspended').click();
  await expect(users.status('Sofia Anders', 'Active')).toBeVisible();
  await users.status('Sofia Anders', 'Active').click();
...
test('QF-STATUS-06 Mutation preserves active filters and selection', async ({ users }) => {
  ...
  await users.status('Sofia Anders', 'Suspended').click();
  await expect(users.status('Sofia Anders', 'Active')).toBeVisible();
  await expect(users.roleFilter).toHaveValue('Manager');
  await expect(users.search).toHaveValue('Sofia');
