...
test('QF-DEFECT-01 Retained audit search must still filter after navigation @regression', async ({ audit, shell }) => {
...
test('QF-DEFECT-02 Exercise 02 validation assertions must locate visible errors uniquely @regression', async ({ shell, challenges, solutions, users, userDialog: form }) => {
...
test('QF-DEFECT-03 Exercise 07 Updating assertion must uniquely locate the status control @regression', async ({ shell, challenges, solutions, users, page }) => {
...
test('QF-DEFECT-05 Keep this lab session must retain demo sign-in on reload @regression', async ({ login, shell, users, page }) => {
...
test('QF-DEFECT-06 Exercise 08 Ready assertion must exclude unrelated hidden content @regression', async ({ shell, challenges, solutions, audit, page }) => {
