...
test('QF-AUTH-01 Login screen exposes accessible controls and demo credentials @smoke', async ({
  login,
  page,
}) => {
  await login.open();
...
for (const [id, email, password, emailError, passwordError] of [
  ...
  test(`QF-AUTH-${id} Login validates an individually missing ${id === '03' ? 'email' : 'password'}`, async ({
    login,
  }) => {
    await login.open();
...
for (const [id, email, password, label] of [
  ...
  test(`QF-AUTH-${id} Login rejects ${label}`, async ({ login, users }) => {
    ...
    await expect(login.alert).toHaveText(
      'The email or password is incorrect. Use the demo credentials above.',
    );
    await expect(login.submit).toBeEnabled();
    await expect(users.heading).toBeHidden();
  });
}

test('QF-AUTH-08 Valid login shows a disabled pending state then Users @smoke', async ({
  login,
  users,
  page,
}) => {
...
test('QF-AUTH-11 Remember checkbox can be checked and unchecked', async ({ login }) => {
  ...
  await expect(login.remember).toBeChecked();
  await login.remember.uncheck();
  await expect(login.remember).not.toBeChecked();
});
...
test('QF-AUTH-13 Sign out hides the application and allows a new login', async ({
  login,
  users,
  shell,
  page,
  ...
  await expect(shell.toast('Signed out')).toBeVisible();
  await login.signIn();
  await expect(users.heading).toBeVisible();
});

test('QF-AUTH-14 Hash navigation alone does not reveal the demo application', async ({
  page,
  login,
  users,
}) => {
...
test('QF-AUTH-15 Credential copy failure offers an observable fallback', async ({ page, login, shell }) => {
  ...
  await login.open();
  await page.getByRole('button', { name: environment.email, exact: true }).click();
  await expect(shell.toast('Select and copy the value manually')).toBeVisible();
});

test('QF-AUTH-16 Credential copy sends the selected value to the clipboard API', async ({
  page,
  login,
  shell,
}) => {
    ...
    Object.defineProperty(navigator, 'clipboard', {
      value: {
        writeText: async (text: string) => {
          document.documentElement.dataset.copiedValue = text;
        },
      },
    }),
  );
