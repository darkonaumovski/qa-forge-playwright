...
test('QF-AUDIT-01 Audit initially shows the first four events in order @smoke', async ({ audit }) => {
  ...
  await expect(audit.loadMore).toBeVisible();
});

test('QF-AUDIT-02 Load more shows a disabled loading state then all eight events', async ({ audit }) => {
  await audit.loadMore.click();
  await expect(audit.loadMore).toHaveText('Loading…');
  await expect(audit.loadMore).toBeDisabled();
  await expect(audit.details).toHaveText(auditEvents);
...
test('QF-AUDIT-03 Debounced role search exposes Filtering then Ready @smoke', async ({ audit }) => {
  ...
  await expect(audit.state).toHaveText('Filtering…');
  await expect(audit.state).toHaveText('Ready');
...
test('QF-AUDIT-05 No-match state recovers when the search is cleared', async ({ audit }) => {
  await audit.search.fill('nonexistent');
  await expect(audit.empty).toBeVisible();
  await expect(audit.events).toHaveCount(0);
  await expect(audit.loadMore).toBeHidden();
  await audit.search.clear();
  await expect(audit.details).toHaveText(auditEvents.slice(0, 4));
  await expect(audit.loadMore).toBeVisible();
});
...
test('QF-AUDIT-06 Rapid input cancels stale debounce work', async ({ audit, page }) => {
  ...
  await audit.search.fill('MFA');
  await page.clock.runFor(299);
  await expect(audit.state).toHaveText('Filtering…');
  await expect(audit.details).toHaveText(auditEvents.slice(0, 4));
  await page.clock.runFor(1);
  await expect(audit.state).toHaveText('Ready');
  await expect(audit.details).toHaveText([auditEvents[6]]);
});

test('QF-AUDIT-07 Search covers events that have not yet been loaded', async ({ audit }) => {
  await audit.search.fill('archived');
  await expect(audit.details).toHaveText([auditEvents[7]]);
});

test('QF-AUDIT-08 Clearing a filter preserves the expanded event limit', async ({ audit }) => {
  await audit.loadMore.click();
  await expect(audit.events).toHaveCount(8);
  await audit.search.fill('role');
  await expect(audit.events).toHaveCount(1);
  await audit.search.clear();
  await expect(audit.details).toHaveText(auditEvents);
  await expect(audit.loadMore).toBeHidden();
});
...
test('QF-AUDIT-09 Export CSV produces the documented simulation notification', async ({ audit, shell }) => {
  await audit.export.click();
  await expect(shell.toast('CSV export prepared — assertion target reached')).toBeVisible();
