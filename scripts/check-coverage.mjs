...
for (const id of ids) {
  ...
  if (projects.length !== 3 || new Set(projects).size !== 3)
    errors.push(`${id}: expected exactly one case per browser project`);
...
console.log(
  `${planned.length}/${planned.length} planned scenarios automated across three browsers; no duplicate IDs.`,
);
